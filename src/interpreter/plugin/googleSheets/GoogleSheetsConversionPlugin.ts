...
export class GoogleSheetsConversionPlugin extends FunctionPlugin implements FunctionPluginTypecheck<GoogleSheetsConversionPlugin> {
  ...
  /**
   * TO_DOLLARS(number) — returns a currency-typed number (displays as USD).
   *
   * Google Sheets returns the value as a number with currency display metadata,
   * not as a formatted string. The host application controls the rendered format.
   */
  ...
  /**
   * TO_PERCENT(number) — returns a percent-typed number.
   *
   * Google Sheets returns the value as a number with percent display metadata,
   * not as a formatted string. The host application controls the rendered format.
   */
